...
class ModelParameter:
    ...
    def __init__(self, param_name, dimension1, default_val, min_val, max_val, steps = 20):
...
class ModelInputs:
    ...
    def add_parameter(self, param_name, dimension1, default_val, min_val, max_val, steps=20):
        self.parameters[param_name] = ModelParameter(param_name, dimension1, default_val, min_val, max_val, steps)
    ...
    def load_parameters_excel(self, excel_path, sheet_name):
        ...
        for index, row in df.iterrows():
            if row['TuningSwitch'].lower() == 'on':  # Only load parameters with TuningSwitch 'on'
                self.add_parameter(param_name=row['Drivers'], dimension1=row["DIMENSION1"], default_val=row['Default'], 
    ...
    def get_param_dim(self):
        ...
        for param in self.parameters:
            param_dim_dict[param] = self.parameters[param].dimension
        return param_dim_dict
